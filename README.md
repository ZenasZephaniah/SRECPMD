...
1.  **Request:** The administrator attempts to access the secure dashboard URL.
2.  **Authentication:** The Middleware intercepts the request to verify the session token. If invalid, the user is redirected to the login screen.
3.  **Fetch:** Upon successful authentication, the server connects directly to the MongoDB database to fetch the latest product data using Server-Side Rendering.
4.  **Render:** The page is generated on the server and sent as fully populated HTML to the browser, ensuring immediate content visibility.
5.  **Interact:** The administrator adds, edits, or deletes products. The server processes these requests and revalidates the data cache to update the UI instantly without a full page reload.
